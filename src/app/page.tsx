...
// === STYLE TOKENS (match your sample) ===
const sectionStyle =
...
// ---------- FilePicker (custom styled file input) ----------
function FilePicker({
  title,
  accept,
  onFile,
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <label
        htmlFor={id}
        className={`${actionBtn} cursor-pointer inline-flex items-center gap-2`}
      >
        {title}
      </label>
...
export default function Page() {
  ...
  function toggleBool(userId: string, key: keyof Row) {
    setRows((prev) =>
      prev.map((r) =>
        r['USER ID'] === userId ? { ...r, [key]: (r[key] ? 0 : 1) as any } : r
      )
    );
  }
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      const byCols = Object.entries(filters).every(([k, val]) => {
        if (!val) return true;
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <section className={sectionStyle}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            <div>
              ...
              <div className={`${valueStyle} text-2xl sm:text-4xl`}>
                {overview.total}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className={`${valueStyle} text-2xl sm:text-3xl`}>
                {nf.format(overview.commSum)}
              </div>
            </div>
            <div>
              <div className={labelStyle}>Prelievi Totali</div>
              <div className={`${valueStyle} text-2xl sm:text-3xl`}>
                {nf.format(overview.wdrSum)}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className={sectionStyle}>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-3">
            ...
            />
            <button onClick={resetFilters} className={actionBtn}>
              Reset Filters
            </button>
          ...
          {rows.length === 0 ? (
            <div className="text-gray-400">
              Carica i file per vedere i risultati.
            </div>
          ) : (
              ...
              <table className="w-full table-compact text-sm">
                <thead>
                  <tr className="bg-white/5 sticky top-0 z-10">
                    {columns.map((k) => (
                      <th
                        key={k}
                        className="text-left whitespace-nowrap py-2 px-3"
                      >
                        {k}
                      </th>
                    ))}
                    ...
                    {columns.map((k) => (
                      ...
                      >
                        <input
                          ...
                          value={filters[k] || ''}
                          onChange={(e) =>
                            setFilter(k as keyof Row, e.target.value)
                          }
                        />
                      </th>
                    ))}
                ...
                <tbody>
                  {filtered.map((r) => (
                    <tr
                      key={r['USER ID']}
                      className="odd:bg-white/0 even:bg-white/[0.02] hover:bg-white/[0.04]"
                    >
                      {columns.map((k) => {
                        const v = (r as any)[k];
                        const isBool = [
                        ...
                        if (isBool) {
                          return (
                            <td key={k} className="whitespace-nowrap py-2 px-3">
                              <button
                                className={`px-2 py-1 rounded-md text-xs font-semibold ${
                                  v ? 'bg-emerald-600/70' : 'bg-zinc-700/70'
                                }`}
                                onClick={() =>
                                  toggleBool(r['USER ID'], k as keyof Row)
                                }
                              >
                                {v ? '1' : '0'}
                              </button>
